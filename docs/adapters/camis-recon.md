...
- **No equipment step.** Accommodation availability reads correctly with **no equipment params at all**; the frontcountry equipment ids (`-32768` "Small Tent" etc., §4 / THR-129 Finding C) are a semantic no-op for cat 1 (byte-identical results with and without them). `BaseCamisAdapter` now skips the equipment extras for the categories in `_NON_EQUIPMENT_BOOKING_CATEGORY_IDS` (Parks Canada → `{1}`); every other category keeps the exact THR-129 shape.
