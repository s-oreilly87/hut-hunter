...
def upgrade() -> None:
    ...
    Adds `status` as a VARCHAR with a server-side default of 'paused' so any
    existing rows backfill cleanly without an explicit UPDATE. Then drops the
    now-redundant `is_active` boolean.
    """
    op.add_column(
        'watchjob',
        sa.Column(
